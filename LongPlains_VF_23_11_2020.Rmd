...
weeds <- weeds %>%  dplyr::filter(!is.na(pre_post))
...
#str(weeds)
weeds <- weeds %>% dplyr::select(pre_post, location_2, total_weeds, "Rye Grass", heads)

weeds_summary <- weeds %>% group_by(pre_post, location_2) %>%
  summarise(
