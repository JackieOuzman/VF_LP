...
subset_biomass_ave <- biomass_ave %>%
  filter(Date == as.Date("2020-10-26")& Paddock == "VF1" |
...
The stimuli data is supplied as a 'cummulative value'.
I would like it to be plotted per day and fence.

I could report the cummulative audio pulses and cummulative pulse pulses for the same time each day of the trial.
...
These values are not cummulative.
...
Jackie: The data we are supplied with from agenrsn is cumulativeShockCount and  cumulativeAudioCount
I have pulled out the max value for shocks and audio for each animal.
then calulated new variable (per anaimal) prop of audio / total cues:
...
mean weight gain = 24.03 SE 1.89
...
Agensen have assigned a code 1922f to this fence, not that there are some data entries 
in the correct time but if they dont have this fence code I have not included them.
...
fence 2 beacme active at 2020-10-25 10:57:00 and the first data log with fence code 1922f and VF area 2 on the 25th
was at 2020-10-25 16:27:00 it took the animals almost 6:30 to cross the VF1 into the new grazing area.

fence 3 beacme active at 2020-10-30 08:07:00 and the and the first data log with fence code 11eab and VF area 3 on the 30th
was at 2020-10-30 08:07:15 it took the animals no time to cross the VF2 into the new grazing area.

This seem a bit too fast, but dont forget that logs only occur every 10 mins.
But I will double check this.

fence 3 beacme de active at 2020-11-03 11:56:00 according to the notes.
no fence became active after it was deactivated so I have no way of filter the data entries.
I have pulled out data on the 2020-11-03 and removed any entries with another fence code, or that are not in the VF4 area.
But this is meaningless.

There are data entries that exist that dont have fence codes, the fence may be deactiave etc but agersen still capture the data.
For the VF I only used data entreis that have this fence code eg no deactive VF data is used, but I cant do this when all the fences are deactivated.

So I cant report on when the first animal moved into VF area 4 when the fence became deactivated


```{r write up GPS plots first animal  25th, echo=FALSE, message=FALSE, warning=FALSE}
animal_move_25_30_03
```