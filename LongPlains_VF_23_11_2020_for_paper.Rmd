...
```{r biomass_location extract, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
All corrlation and R2 are terrible 
...
```{r biomass_location extract plots, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
weeds_summary_total <- weeds_summary %>% 
  mutate(av_total_weeds = round(av_total_weeds,2),
          st_dev_total_weeds = round(st_dev_total_weeds,2),
...
precentage weight gain

```{r percen weight gain, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r mean daily weight gain breed and sex, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r animal wt gains  per animals per day, echo=TRUE, message=FALSE, warning=FALSE}
# animal_wt 
...
# total weight gain for animal per day
weight_gain-per_animals_per_day <- animal_wt %>% 
  group_by(`Mob Name`) %>% 
  summarise(`total weight gain` = sum(`Weight gain`),
            `total weight gain SD` = sd(`Weight gain`,  na.rm = TRUE),
            `total weight gain SE` = `total weight gain SD` / sqrt(n()))
